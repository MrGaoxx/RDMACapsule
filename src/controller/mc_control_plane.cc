...
SwitchTableWritter::SwitchTableWritter(uint32_t addr)
    : switch_addr(0x0a00007b){
        switch_addr=addr;
}
SwitchTableWritter::SwitchTableWritter()
    : switch_addr(0x0a00007b){}

uint32_t SwitchTableWritter::get_switch_addr(){
    return switch_addr;
}
...
uint32_t SwitchTableWritter::multicast_group_add(uint32_t src_addr, uint32_t dqp_num, uint32_t sqp_num, uint32_t dst_addr1, uint32_t dqp1, uint32_t dst_addr2, uint32_t dqp2, uint32_t dst_addr3, uint32_t dqp3){
    ...
    if (Py_IsInitialized())
    {
        ...
        if (pModule)
        {
            PyObject* pArgs = PyTuple_New(10);//函数调用的参数传递均是以元组的形式打包的,2表示参数个数。如果AdditionFc中只有一个参数时，写1就可以了。这里只先介绍函数必须有参数存在的情况。
...
void SwitchTableWritter::free_multicast_group_id(uint32_t mcg_id)
{
