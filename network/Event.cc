...
class C_handle_notify : public EventCallback {
   ...
   public:
    C_handle_notify(EventCenter *c, Configure *cc) : center(c), config(cc) {}
        ...
        do {
            ...
            if (r < 0) {
                if (ceph_sock_errno() != EAGAIN)
                    ldout(config, 1) << __func__ << " read notify pipe failed: " << cpp_strerror(ceph_sock_errno()) << dendl;
            }
        } while (r > 0);
    }
};
...
int EventCenter::init(int nevent, unsigned center_id, const std::string &type) {
...
#else
#ifdef HAVE_KQUEUE
        driver = new KqueueDriver(config);
#else
        driver = new SelectDriver(config);
#endif
#endif
    }

    if (!driver) {
        lderr(config) << __func__ << " failed to create event driver " << dendl;
        return -1;
    }

    int r = driver->init(this, nevent);
    if (r < 0) {
        lderr(config) << __func__ << " failed to init event driver." << dendl;
        return r;
    }
    ...
    if (pipe_cloexec(fds, 0) < 0) {
        ...
        lderr(config) << __func__ << " can't create notify pipe: " << cpp_strerror(e) << dendl;
        return -e;
    }
...
void EventCenter::set_owner() {
    owner = pthread_self();
    ldout(config, 2) << __func__ << " center_id=" << center_id << " owner=" << owner << dendl;
    if (!global_centers) {
        global_centers =
...
int EventCenter::create_file_event(int fd, int mask, EventCallbackRef ctxt) {
    ...
    if (fd >= nevent) {
        ...
        ldout(config, 20) << __func__ << " event count exceed " << nevent << ", expand to " << new_size << dendl;
        r = driver->resize_events(new_size);
        if (r < 0) {
            lderr(config) << __func__ << " event count is exceed." << dendl;
            return -ERANGE;
        }
    ...
    ldout(config, 20) << __func__ << " create event started fd=" << fd << " mask=" << mask << " original mask is " << event->mask << dendl;
    ...
    if (r < 0) {
        ...
        lderr(config) << __func__ << " add event failed, ret=" << r << " fd=" << fd << " mask=" << mask << " original mask is " << event->mask
                      << dendl;
        ceph_abort_msg("BUG!");
        return r;
    }
    ...
    ldout(config, 20) << __func__ << " create event end fd=" << fd << " mask=" << mask << " current mask is " << event->mask << dendl;
    return 0;
}
...
void EventCenter::delete_file_event(int fd, int mask) {
    ...
    if (fd >= nevent) {
        ldout(config, 1) << __func__ << " delete event fd=" << fd << " is equal or greater than nevent=" << nevent << "mask=" << mask << dendl;
        return;
    }
    EventCenter::FileEvent *event = _get_file_event(fd);
    ldout(config, 30) << __func__ << " delete event started fd=" << fd << " mask=" << mask << " original mask is " << event->mask << dendl;
    ...
    ldout(config, 30) << __func__ << " delete event end fd=" << fd << " mask=" << mask << " current mask is " << event->mask << dendl;
}
...
uint64_t EventCenter::create_time_event(uint64_t microseconds, EventCallbackRef ctxt) {
    ...
    ldout(config, 30) << __func__ << " id=" << id << " trigger after " << microseconds << "us" << dendl;
...
void EventCenter::delete_time_event(uint64_t id) {
    ceph_assert(in_thread());
    ldout(config, 30) << __func__ << " id=" << id << dendl;
    if (id >= time_event_next_id || id == 0) return;

    auto it = event_map.find(id);
    if (it == event_map.end()) {
        ldout(config, 10) << __func__ << " id=" << id << " not found" << dendl;
        return;
    }
...
void EventCenter::wakeup() {
    ...
    ldout(config, 20) << __func__ << dendl;
    char buf = 'c';
// wake up "event_wait"
#ifdef _WIN32
    ...
    if (n < 0) {
        if (ceph_sock_errno() != EAGAIN) {
            ldout(config, 1) << __func__ << " write notify pipe failed: " << cpp_strerror(ceph_sock_errno()) << dendl;
            ceph_abort();
        }
    }
}
...
int EventCenter::process_time_events() {
    ...
    using ceph::operator<<;
    ldout(config, 30) << __func__ << " cur time is " << now << dendl;
    ...
    while (!time_events.empty()) {
        ...
        if (now >= it->first) {
            ...
            event_map.erase(id);
            ldout(config, 30) << __func__ << " process time event: id=" << id << dendl;
            processed++;
            cb->do_request(id);
        } else {
            break;
...
int EventCenter::process_events(unsigned timeout_microseconds, common::timespan *working_dur) {
    ...
    ldout(config, 30) << __func__ << " wait second " << tv.tv_sec << " usec " << tv.tv_usec << dendl;
    ...
    for (int event_id = 0; event_id < numevents; event_id++) {
        ...
        ldout(config, 30) << __func__ << " event_wq process is " << fired_events[event_id].fd << " mask is " << fired_events[event_id].mask << dendl;
    }
    ...
    if (external_num_events.load()) {
        ...
        while (!cur_process.empty()) {
            ...
            ldout(config, 30) << __func__ << " do " << e << dendl;
            e->do_request(0);
...
void EventCenter::dispatch_event_external(EventCallbackRef e) {
    ...
    ldout(config, 30) << __func__ << " " << e << " pending " << num << dendl;
}
