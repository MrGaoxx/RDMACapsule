...
class ServerSocketImpl {
   public:
    ...
    ServerSocketImpl(unsigned type, unsigned slot) : addr_type(type), addr_slot(slot) {}
...
class ConnectedSocket {
   ...
   public:
    ...
    ConnectedSocket(){};
    /// \cond internal
    explicit ConnectedSocket(std::unique_ptr<ConnectedSocketImpl> csi) : _csi(std::move(csi)) {}
    /// \endcond
    ~ConnectedSocket() {
        if (_csi) _csi->close();
    }
...
class ServerSocket {
   ...
   public:
    ...
    ServerSocket() {}
    /// \cond internal
    explicit ServerSocket(std::unique_ptr<ServerSocketImpl> ssi) : _ssi(std::move(ssi)) {}
    ~ServerSocket() {
    ...
    int accept(ConnectedSocket *sock, const SocketOptions &opt, entity_addr_t *out, Worker *w) { return _ssi->accept(sock, opt, out, w); }
